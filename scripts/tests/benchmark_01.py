...
def setup_modin():
    """Initialize Modin with conservative Dask configuration"""
    cfg.Engine.put("dask")
    cfg.NPartitions.put(4)  # Reduce partitions for stability
    import dask
    dask.config.set({
...
def run_modin_operation(func, csv_path):
    """Run a Modin operation with a conservative Dask cluster"""
    cluster = LocalCluster(
        n_workers=2,  # Reduce workers for stability
