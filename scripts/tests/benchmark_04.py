...
def setup_modin():
    """Initialize Modin with optimized Dask configuration for Windows"""
    cfg.Engine.put("dask")
    cfg.NPartitions.put(8)  # Use more partitions for parallelism
    import dask
    dask.config.set({
        "distributed.worker.memory.target": 0.7,  # 70% of total memory
        ...
        "distributed.scheduler.worker-saturation": 1.0,  # Allow full CPU usage
        "distributed.worker.threads.nthreads": 4,  # Use multiple threads per worker
    })
    logging.getLogger("distributed").setLevel(logging.CRITICAL)
    warnings.filterwarnings("ignore")

def run_modin_operation(func, csv_path):
    """Run a Modin operation with a single Dask worker and multiple threads for Windows"""
    cluster = LocalCluster(
        n_workers=1,  # Single worker for Windows
        threads_per_worker=4,  # Use multiple threads
        memory_limit="6GB",  # Increase memory limit
    ...
    try:
        ...
        print(f"Modin operation failed: {e}")
        return None
    finally:
        client.close()
        cluster.close()
...
def modin_filter_group(csv_path):
    ...
    result = run_modin_operation(_modin_filter_group, csv_path)
    if result is None:
        print("Modin failed, falling back to Pandas")
...
def modin_stats(csv_path):
    ...
    result = run_modin_operation(_modin_stats, csv_path)
    if result is None:
        print("Modin failed, falling back to Pandas")
...
def modin_complex(csv_path):
    def _modin_complex(path):
        ...
        result["bytes_rank"] = result.groupby("event_type")["bytes"].rank(method="dense", ascending=False)
        return result[result["bytes_rank"] <= 10]
    result = run_modin_operation(_modin_complex, csv_path)
    if result is None:
        print("Modin failed, falling back to Pandas")
...
def modin_timeseries(csv_path):
    ...
    result = run_modin_operation(_modin_timeseries, csv_path)
    if result is None:
        print("Modin failed, falling back to Pandas")
